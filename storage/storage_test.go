...
func TestIntegrationReloadingPartialTorrents(t *testing.T) {
	...
	targetDir := t.TempDir()
	workDir := t.TempDir()
	storage := New(targetDir, workDir, info)
	err := storage.Load()
	if err != nil {
		t.Fatal(err)
	}
	...
	// Piece has no data
	if _, err = storage.GetBlock(0, 0, BLOCK_LENGTH); err == nil {
		t.Fatal("want: piece not coalesced error")
	}
	// Invalid block puts
	if err = storage.PutBlock(0, 1024, pieces[0][:BLOCK_LENGTH/2]); err == nil {
		t.Fatal("want: block alignment error")
	}
	if err = storage.PutBlock(0, 0, pieces[0][:BLOCK_LENGTH/2]); err == nil {
		t.Fatal("want: block length error")
	}
	// Piece has partial data
	if err = storage.PutBlock(0, 0, pieces[0][:BLOCK_LENGTH]); err != nil {
		t.Fatal(err)
	}
	if _, err = storage.GetBlock(0, 0, BLOCK_LENGTH); err == nil {
		t.Fatal("want: piece not coalesced error")
	}
	if err = storage.PutBlock(0, BLOCK_LENGTH, pieces[0][BLOCK_LENGTH:]); err != nil {
		t.Fatal(err)
	}
	// Piece has complete data
	if !storage.GetPiece(0).coalesced {
	...
	data, err := storage.GetBlock(0, 0, BLOCK_LENGTH)
	if err != nil {
		t.Fatal(err)
	}
	...
	storage.Torrent = newTorrent(info)
	err = storage.Load()
	if err != nil {
		t.Fatal(err)
	}
	...
	if storage.GetPiece(1).coalesced {
	...
	for i := 1; i < len(pieces); i++ {
		...
		if err = storage.coalesceBlocks(piece); err != nil {
			t.Fatal(err)
		}
	}
	// Pieces have complete data
	for i := range len(pieces) {
		if !storage.GetPiece(i).coalesced {
...
func TestIntegrationSingleFileTorrent(t *testing.T) {
	...
	for i := 0; i < len(pieces); i++ {
		if err = storage.PutBlock(i, 0, pieces[i][:BLOCK_LENGTH]); err != nil {
			t.Fatal(err)
		}
		if err = storage.PutBlock(i, BLOCK_LENGTH, pieces[i][BLOCK_LENGTH:]); err != nil {
			t.Fatal(err)
		}
	}
	if !torrent.coalesced {
	...
	checkBlock := func(index, begin, length int) {
		t.Helper()
		data, err := storage.GetBlock(index, begin, length)
		if err != nil {
	...
	for i := 0; i < len(pieces); i++ {
		...
		go func(i int) {
			...
			for curr := 0; curr < len(pieces[i]); curr += BLOCK_LENGTH {
				...
				if err = storage.PutBlock(i, curr, pieces[i][curr:curr+blockLength]); err != nil {
					t.Error(err)
				}
			}
			for curr := 0; curr < len(pieces[i]); curr += BLOCK_LENGTH {
...
func BenchmarkCoalesceSingleFile(b *testing.B) {
	for range b.N {
		...
		b.StartTimer()
		if err := storage.coalescePieces(); err != nil {
			b.Fatal(err)
		}
